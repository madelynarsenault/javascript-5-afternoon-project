...
// Write a constructor function called User. This function should take in 4 parameters called name, age, email, and savedPosts in that order. Name and email will be strings, age will be a number and savedPosts will be an array of objects. These objects will each have 3 properties:
...
//It should take in three parameters: id (a number), title (a string) and rating (a number). Use these parameters to create a new object and add it to the savedPosts array. Make sure to name the properties the same as described previously (id, title, rating).
...
User.prototype.addSavedPost = function(id, rating, title){
const newPost = {
  id: id,
  title: title,
  rating: rating
...
User.prototype.changePostRating = function(id2, newRating2){
  for (let i = 0; i < this.savedPosts.length; i++){
    if (this.savedPosts[i].id === id2){
      this.savedPosts[i].rating = newRating2
    }
  }
}
